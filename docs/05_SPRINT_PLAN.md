...
- **Status C3 — selesai (lihat ADR-010):**
  - `POST /api/v1/payments` (201, `Idempotency-Key` wajib): validasi → snapshot receivable → alokasi engine C2 → simpan `payment`/`payment_allocation` → resolusi angsuran → posting jurnal, semuanya dalam satu `@Transactional` application service. `paid_at` selalu dari `Clock` (Asia/Jakarta), tidak pernah dari request.
  - **Seam antar-modul baru:** `contract.application.InstallmentReceivablePort` (`loadReceivableSnapshot` + `applyPaymentResolution`), diimplementasikan `InstallmentReceivableService`. `payment` tidak pernah menyentuh tabel/entity `contract`; port yang melempar 404 `CONTRACT_NOT_FOUND` / 409 `CONTRACT_STATE_INVALID` dan port yang menurunkan `InstallmentStatus`/`paid_at` (aturan state tetap milik pemilik data). Edge `payment → contract (application interface)` diresmikan di TS §1.
  - `Installment.applyPayment` (mutator pertama aggregate): `paid_amount` naik, `paid_at` di-set hanya saat resolusi pertama (instant pembayaran pertama, bukan terakhir), `PARTIALLY_PAID` vs `PAID` diturunkan dari `InstallmentBalance.outstanding()`, dan angsuran `SETTLED`/`WRITTEN_OFF` tidak pernah diturunkan statusnya.
  - Jurnal pembayaran: satu entry `PAYMENT` per pembayaran, debit `KAS` = jumlah pembayaran, kredit agregat `PIUTANG_DENDA → PIUTANG_BUNGA → PIUTANG_POKOK → TITIPAN_NASABAH` (nol dilewati), `contract_id` di setiap baris (TS §3).
  - **Idempotency** memakai mekanisme B5 apa adanya (endpoint scope `POST /api/v1/payments`): retry identik mengembalikan `response_json` tersimpan tanpa baris `payment`/jurnal kedua, key sama + body beda = 409, key salah = 400. Claim ada di dalam transaksi bisnis sehingga operasi gagal me-rollback claim dan retry bisa memperbaiki body. **Cleanup key kedaluwarsa tetap milik F4** (bukan C3) — TS §2.5 diperbaiki.
  - `contract_credit` **tidak** ditulis (milik E3); kelebihan hanya menjadi baris `EXCESS` + kredit `TITIPAN_NASABAH`. Void (E4), billing/recognition (C4), dan waive (E5) tidak disentuh.
  - **Tanpa migrasi** (DDL `payment`/`payment_allocation` sudah ada di V1, trigger V3 sudah ada) dan tanpa perubahan OpenAPI di luar endpoint baru.
  - Fix pendukung: `spring.jackson.time-zone=Asia/Jakarta` (TS §2.0) supaya respons replay idempotent identik dengan respons pertama — Jackson 3 merender `OffsetDateTime` di zone mapper, sehingga tanpa ini replay mengembalikan instant yang sama sebagai `…Z`, bukan `…+07:00`.
  - Test: 23 test baru — `PaymentApiIT` (14, MockMvc + PostgreSQL 16 asli, commit sungguhan agar trigger V3 jalan; termasuk overpayment/EXCESS tanpa `contract_credit`, pembayaran sebelum due date, pembayaran telat yang menghabiskan denda, replay idempotent, konflik key, matriks error 400/404/409, dan snapshot korup → rollback penuh), `PaymentIdempotencyIT` (4, service level: retry identik bersamaan, claim rollback saat operasi gagal), `InstallmentPaymentResolutionTest` (5, unit murni resolusi angsuran). Suites `*IT` memakai `FixedClock` via `PaymentClockTestConfiguration` agar `paid_at` dan nomor dokumen asertif.
